...
abstract class ActionBase(private val action: String, private val presetInstruction: String) : AnAction() {
    ...
    private fun submit(project: Project, input: String, psiFile: PsiFile?) {
    ...
    private fun findCefChatPanel(project: Project): io.codepilot.plugin.toolwindow.CefChatPanel? {
        return io.codepilot.plugin.toolwindow.CefChatPanelRegistry.getInstance(project)
    }

    /**
     * Call `POST /v1/actions/{action}` with SSE and stream results to the chat panel.
     */
    private fun callActionSse(panel: io.codepilot.plugin.toolwindow.CefChatPanel, input: String, psiFile: PsiFile?) {
