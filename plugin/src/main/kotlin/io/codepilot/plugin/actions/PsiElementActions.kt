...
abstract class PsiElementActionBase(
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        tw.show {
            ApplicationManager.getApplication().invokeLater {
                ...
                if (panel != null) {
                    panel.dispatchToWeb("action_start", mapOf(
                        "action" to action,
    ...
    private fun callActionSse(
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                val url = (settings.state.backendBaseUrl.trimEnd('/') + "/v1/actions/$action")
                    .toHttpUrl()
                ...
                http.openSse(request, object : okhttp3.sse.EventSourceListener() {
...
class PsiGenTestAction : PsiElementActionBase("gentest", "Generate unit tests covering happy-path and edge cases. Return a Patch JSON creating the test file.")

/** Generate documentation for the selected class or method. */
class PsiGenDocAction : PsiElementActionBase("gendoc", "Produce developer documentation (KDoc/Javadoc) for the following element. Return a Patch JSON with the comments inserted.")

/** Generate comments for the selected class or method. */
class PsiCommentAction : PsiElementActionBase("comment", "Add idiomatic inline comments and doc comments without altering executable code. Return a Patch JSON.")