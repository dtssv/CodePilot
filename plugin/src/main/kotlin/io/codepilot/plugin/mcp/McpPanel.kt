...
class McpPanel(private val project: Project) {
    ...
    private fun buildInstallPane(): JComponent {
        val form = JPanel(GridBagLayout()).apply {
            ...
            val hintGbc = GridBagConstraints().apply {
                ...
                fill = GridBagConstraints.HORIZONTAL; insets = Insets(0, 4, 8, 4)
            }
            add(JLabel(
            ...
            val nameLabelGbc = GridBagConstraints().apply {
                gridx = 0; gridy = 1; anchor = GridBagConstraints.WEST; insets = Insets(4, 4, 4, 4)
            }
            ...
            // JSON label
            val jsonLabelGbc = GridBagConstraints().apply {
                gridx = 0; gridy = 2; anchor = GridBagConstraints.NORTHWEST; insets = Insets(4, 4, 4, 4)
            }
            add(JLabel("JSON Config:"), jsonLabelGbc)

            // JSON text area
            val jsonGbc = GridBagConstraints().apply {
                gridx = 1; gridy = 2; weightx = 1.0; weighty = 1.0
            ...
            btnPanel.add(JButton("Clear").apply { addActionListener {
                jsonInput.text = EXAMPLE_JSON; nameField.text = ""
            } })
            val btnGbc = GridBagConstraints().apply {
                gridx = 1; gridy = 3; weightx = 1.0; fill = GridBagConstraints.HORIZONTAL
    ...
    private fun installFromJson() {
        ...
        try {
            ...
            if (entries.isEmpty()) {
                Messages.showErrorDialog(project, "Could not parse any MCP server from the JSON.\nExpected format: {\"mcpServers\":{\"name\":{\"command\":\"...\",\"args\":[...]}}}", "CodePilot")
                return
            }
            var installed = 0
    ...
    private fun parseSingleServer(name: String, node: com.fasterxml.jackson.databind.JsonNode): LocalMarketplaceStore.McpEntry {
    ...
    private fun refreshInstalled() {
        ...
        val allIndex = try {
            val idx = mapper.readTree(java.nio.file.Files.readString(
    ...
    private fun startSelected() {
        ...
        if (item == null) { Messages.showWarningDialog(project, "Select a server to start.", "CodePilot"); return }
        try {
            mcpManager.start(item.entry.id, McpProcessManager.McpLaunchSpec(
                id = item.entry.id, argv = item.entry.argv, cwd = item.entry.cwd, env = item.entry.env
            ))
    ...
    private fun stopSelected() {
        val item = installedList.selectedValue
        if (item == null) { Messages.showWarningDialog(project, "Select a server to stop.", "CodePilot"); return }
    ...
    private fun uninstallSelected() {
        val item = installedList.selectedValue
        if (item == null) { Messages.showWarningDialog(project, "Select a server to uninstall.", "CodePilot"); return }
        if (Messages.showOkCancelDialog(project, "Uninstall MCP server '${item.entry.id}'?", "CodePilot",
                "Uninstall", "Cancel", Messages.getWarningIcon()) != Messages.OK) return
        mcpManager.stop(item.entry.id)
    ...
    data class McpServerItem(val entry: LocalMarketplaceStore.McpEntry, val running: Boolean)

    private class McpServerRenderer : ListCellRenderer<McpServerItem> {
        override fun getListCellRendererComponent(
            list: JList<out McpServerItem>, value: McpServerItem, index: Int,
            ...
            val title = JLabel("<html><b>${value.entry.id}</b> &nbsp; <font color='${if (value.running) "green" else "gray"}'>$statusIcon</font></html>")
            val cmd = value.entry.argv.joinToString(" ")
            val detail = JLabel("<html><code>${cmd}</code>" +
                if (value.entry.env.isNotEmpty()) "<br>env: ${value.entry.env.keys.joinToString()}" else "" +
                "</html>")
