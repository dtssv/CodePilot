...
class McpProcessManager : Disposable {
    ...
    private fun launch(spec: McpLaunchSpec): Handle {
        ...
        val readerThread = Thread({ pumpStdout(handle, BufferedReader(InputStreamReader(proc.inputStream, StandardCharsets.UTF_8))) }, "mcp-stdout-${spec.id}")
    ...
    private fun pumpStdout(handle: Handle, reader: BufferedReader) {
        reader.use { r ->
            while (handle.process.isAlive) {
                ...
                val node = try { mapper.readTree(line) } catch (_: Throwable) { continue }
    ...
    private fun drain(stream: java.io.InputStream) {
        BufferedReader(InputStreamReader(stream, StandardCharsets.UTF_8)).use { r ->
            while (true) {
                ...
                com.intellij.openapi.diagnostic.Logger.getInstance("McpProcessManager").warn("[mcp-stderr] $line")
            }
        }
    }
    ...
    private class Handle(val process: Process, val writer: java.io.BufferedWriter) {
        val lock = Any()
        ...
        fun deliver(id: Int, body: JsonNode) {
            val fut = pending.remove(id) ?: run {
                dropped.add(body)
                return
            }
            fut.complete(body)
