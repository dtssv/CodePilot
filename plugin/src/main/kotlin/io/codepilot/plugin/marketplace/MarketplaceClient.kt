...
class MarketplaceClient(
    ...
    fun listPackages(type: String? = null, query: String? = null, page: Int = 1, size: Int = 50): CompletableFuture<List<Package>> {
        val url = base()
            .newBuilder()
    ...
    fun manifest(slug: String, version: String): CompletableFuture<Map<String, Any?>> {
        val url = base()
            .newBuilder()
    ...
    fun reportInstall(slug: String, version: String, scope: LocalMarketplaceStore.Scope, source: LocalMarketplaceStore.Source): CompletableFuture<Boolean> {
    ...
    fun reportUninstall(slug: String, version: String, scope: LocalMarketplaceStore.Scope, source: LocalMarketplaceStore.Source): CompletableFuture<Boolean> {
        val req = http.postJson(
            "/v1/mcp/uninstall",
            mapOf("slug" to slug, "version" to version, "scope" to scope.value, "source" to source.value),
        )
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    data class PageEnvelope(val code: Int, val data: PageData?)

    @JsonIgnoreProperties(ignoreUnknown = true)
    data class PageData(val total: Int, val items: List<Package>)
}