...
class AtReferenceProvider(private val project: Project) {
    ...
    fun suggest(query: String, limit: Int = 15): List<Suggestion> {
        if (query.isBlank()) {
            return listOf(
    ...
    fun resolve(type: String, value: String): ReferenceResult? {
        return when (type) {
    ...
    private fun searchFiles(query: String, limit: Int): List<Suggestion> {
        if (query.isBlank()) return emptyList()
        return ReadAction.compute<List<Suggestion>, Throwable> {
            val scope = GlobalSearchScope.projectScope(project)
            val files = FilenameIndex.getAllFilesByExt(project, query.substringAfterLast('.', ""), scope)
    ...
    private fun searchFolders(query: String, limit: Int): List<Suggestion> {
        if (query.isBlank()) return emptyList()
    ...
    private fun searchSymbols(query: String, limit: Int): List<Suggestion> {
        if (query.isBlank()) return emptyList()
        return ReadAction.compute<List<Suggestion>, Throwable> {
            ...
            cache.getClassesByName(query, scope).take(limit / 2).forEach { cls ->
                val path = cls.containingFile?.virtualFile?.path?.removePrefix(project.basePath ?: "")?.trimStart('/') ?: ""
            ...
            cache.getMethodsByName(query, scope).take(limit / 2).forEach { method ->
                ...
                val path = method.containingFile?.virtualFile?.path?.removePrefix(project.basePath ?: "")?.trimStart('/') ?: ""
    ...
    private fun resolveGit(command: String): ReferenceResult? {
        ...
        val result = try {
            val args = com.fasterxml.jackson.databind.ObjectMapper().createObjectNode()
    ...
    private fun searchDocs(query: String, limit: Int): List<Suggestion> {
        ...
        val results = mutableListOf<Suggestion>()
        docsDir.children.filter { !it.isDirectory && it.name.lowercase().contains(query.lowercase()) }
            .take(limit)
            .forEach { vf ->
                val rel = vf.path.removePrefix(projectBase).trimStart('/')
