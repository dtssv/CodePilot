...
object CommitMessageGenerator {
    ...
    fun getStagedDiff(project: Project): String? = runGit(project, "diff", "--cached", "--no-color")

    /** Gets the current branch name. */
    fun getCurrentBranch(project: Project): String? = runGit(project, "rev-parse", "--abbrev-ref", "HEAD")?.trim()

    /** Gets recent commit messages (last 5) for style reference. */
    fun getRecentCommitMessages(project: Project): String? = runGit(project, "log", "--oneline", "-5", "--no-decorate")
