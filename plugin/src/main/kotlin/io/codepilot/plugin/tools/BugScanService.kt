 ...
 */
class BugScanService(private val project: Project) {
    ...
    fun scanOpenFiles(): ScanResult {
        ...
        ApplicationManager.getApplication().runReadAction {
            val editors = com.intellij.openapi.fileEditor.FileEditorManager.getInstance(project).openFiles
            for (vf in editors) {
                try {
        ...
        return ScanResult(
            diagnostics = diagnostics,
            fileCount = com.intellij.openapi.fileEditor.FileEditorManager.getInstance(project).openFiles.size,
            durationMs = System.currentTimeMillis() - startTime,
        )
    }
        ...
        return ApplicationManager.getApplication().runReadAction<Map<String, Any?>?> {
            try {
                ...
                mapOf(
                    ...
                    "diagnostics" to scanResult.diagnostics.map { d ->
