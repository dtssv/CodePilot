...
/**
 * ★ Privacy Mode Filter: When the {@code X-CodePilot-Privacy} header is set to "strict", this
 * filter:
 *
 * <ol>
 *   <li>Blocks requests to telemetry/audit endpoints entirely
 *   <li>Adds {@code X-CodePilot-Data-Retained: none} response header
 *   <li>Strips user-identifiable headers (X-User-Id, X-Tenant-Id) from downstream
 *   <li>Propagates the privacy flag via {@code X-CodePilot-Privacy} to backend services so they can
 *       skip Redis caching, audit writes, and telemetry
 * </ol>
 *
 * <p>When X-Anonymous-Mode is set, the filter allows the request but ensures no user-identifiable
