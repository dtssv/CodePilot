 ...
 *
 * <ul>
 *   <li>default-src 'self' — only load resources from same origin
 *   <li>script-src 'self' 'unsafe-inline' — allow inline scripts for JCEF React UI
 *   <li>style-src 'self' 'unsafe-inline' — allow inline styles
 *   <li>img-src 'self' data: blob: https: — allow data URIs and images
 *   <li>connect-src 'self' ws: wss: https: — allow WebSocket and API calls
 *   <li>object-src 'none' — no plugins
 *   <li>frame-ancestors 'none' — prevent clickjacking
 * </ul>
 */
@Component
