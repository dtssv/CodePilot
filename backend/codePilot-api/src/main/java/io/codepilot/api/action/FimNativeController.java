 ...
 *
 * <p>Provides a dedicated endpoint for code completion models that support native FIM/infilling
 * APIs (e.g., Codestral, DeepSeek-Coder, StarCoder2, Qwen2.5-Coder). These models accept structured
 * prefix/suffix/middle tokens directly without needing prompt engineering.
 *
 * <p>Protocol: - POST /v1/fim/completions — OpenAI-compatible FIM API - Request: { model, prompt,
 * suffix, max_tokens, temperature, stop, n } - Response: SSE stream of { id, choices: [{ index,
 * text, finish_reason }] }
 *
 * <p>Models supporting native FIM: - Codestral (Mistral): prompt/suffix parameters -
 * DeepSeek-Coder: prefix/suffix FIM format - StarCoder2: <fim_prefix>/<fim_suffix>/<fim_middle>
 * tokens - Qwen2.5-Coder: <fim_prefix>/<fim_suffix>/<fim_middle> tokens
 *
 * <p>The endpoint auto-detects the model's FIM format and adapts the request accordingly, falling
 * back to chat-based completion for non-FIM models.
 */
...
public class FimNativeController {
  ...
  public Flux<ServerSentEvent<String>> completions(@RequestBody @Valid FimRequest req) {
    String modelId =
        req.model() != null
            ? req.model()
            : modelService.listModelGroups().stream()
                .filter(g -> g.capabilities() != null && g.capabilities().contains("fim"))
                .map(io.codepilot.core.model.ModelGroup::model)
                .findFirst()
                .orElse(
                    modelService.listModelGroups().isEmpty()
                        ? null
                        : modelService.listModelGroups().getFirst().model());
    return executeFimCompletion(modelId, req.modelSource(), req);
  }

  /**
   * Execute FIM completion by detecting the model's native FIM format and constructing the
   * appropriate request.
   */
  private Flux<ServerSentEvent<String>> executeFimCompletion(
      String modelId, ModelSource modelSource, FimRequest req) {
  ...
  private String detectFimFormat(String modelId) {
    ...
    if (lower.contains("qwen") && lower.contains("coder"))
      return "starcoder"; // Qwen uses same format
    if (lower.contains("codellama")) return "starcoder";
    return "chat"; // Unknown model, fall back to chat-based
  }

  /**
   * Codestral FIM format: Uses prompt + suffix parameters directly. POST /v1/fim/completions with {
   * prompt, suffix }
   */
  ...
  /**
   * DeepSeek-Coder FIM format: Uses prefix + suffix in the FIM-specific format. Wraps in the
   * DeepSeek chat completions API with special FIM markers.
   */
  ...
  /**
   * StarCoder2 / Qwen2.5-Coder FIM format: Uses <fim_prefix>/<fim_suffix>/<fim_middle> token
   * markers.
   */
  ...
  /**
   * Chat-based FIM fallback for models without native FIM support. Uses a standard chat completion
   * with the prefix/suffix context.
   */
  ...
  /**
   * Stream the FIM response as SSE events, normalizing the output format to the OpenAI FIM
  ...
  private Flux<ServerSentEvent<String>> streamFimResponse(
    ...
    return client.chatClient().prompt().user(content).stream()
        .chatResponse()
        .map(
            chatResp -> {
  ...
  /** OpenAI-compatible FIM completion request. Matches the OpenAI /v1/fim/completions API spec. */
