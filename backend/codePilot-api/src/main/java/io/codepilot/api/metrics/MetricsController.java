 ...
 *
 * <p>Metrics: chat/agent/completion requests, tokens, errors, latency histogram,
 * active sessions, Shadow validations, MCP servers.
 */
@RestController
...
public class MetricsController {
    ...
    public ResponseEntity<String> prometheusMetrics() {
        ...
        appendMetric(sb, "codepilot_chat_requests_total", "counter", "Total chat requests", chatRequests.sum());
        appendMetric(sb, "codepilot_agent_requests_total", "counter", "Total agent requests", agentRequests.sum());
        appendMetric(sb, "codepilot_completion_requests_total", "counter", "Total FIM completion requests", completionRequests.sum());
        appendMetric(sb, "codepilot_tokens_input_total", "counter", "Total input tokens", inputTokens.sum());
        appendMetric(sb, "codepilot_tokens_output_total", "counter", "Total output tokens", outputTokens.sum());
        appendMetric(sb, "codepilot_shadow_validations_total", "counter", "Shadow validations", shadowValidations.sum());
        appendMetric(sb, "codepilot_shadow_validation_failures_total", "counter", "Shadow validation failures", shadowValidationFailures.sum());
        appendMetric(sb, "codepilot_active_sessions", "gauge", "Active sessions", activeSessions.get());
        appendMetric(sb, "codepilot_mcp_servers_active", "gauge", "Active MCP servers", mcpServersActive.get());
        appendMetric(sb, "codepilot_uptime_seconds", "gauge", "Uptime seconds", Duration.between(startTime, Instant.now()).getSeconds());
        ...
        for (var entry : errorsByType.entrySet()) {
            ...
            sb.append("codepilot_errors_total{type=\"").append(entry.getKey()).append("\"} ")
    ...
    public ResponseEntity<Map<String, Object>> healthMetrics() {
        ...
        return ResponseEntity.ok(Map.of(
            ...
            "metrics", Map.of(
                ...
                "errors_by_type", errorsByType.entrySet().stream()
    ...
    public void recordAgentRequest() { agentRequests.increment(); }
    public void recordCompletionRequest() { completionRequests.increment(); }
    public void recordInputTokens(int count) { inputTokens.add(count); }
    public void recordOutputTokens(int count) { outputTokens.add(count); }
    ...
    public void recordError(String type) { errorsByType.computeIfAbsent(type, k -> new LongAdder()).increment(); }
    ...
    public void setActiveSessions(long count) { activeSessions.set(count); }
    public void setMcpServersActive(long count) { mcpServersActive.set(count); }
