...
/**
 * Server-side web fetch proxy for @web references. Fetches URL content on behalf of the plugin
 * client to avoid CORS issues and enforce security policies (allowlist, content-size limits).
 */
@RestController
...
public class WebFetchController {
  ...
  public WebFetchController() {
    HttpClient httpClient = HttpClient.create().responseTimeout(FETCH_TIMEOUT).followRedirect(true);

    this.webClient =
        WebClient.builder()
            .clientConnector(new ReactorClientHttpConnector(httpClient))
            .codecs(configurer -> configurer.defaultCodecs().maxInMemorySize(MAX_CONTENT_SIZE))
            .build();
  }
