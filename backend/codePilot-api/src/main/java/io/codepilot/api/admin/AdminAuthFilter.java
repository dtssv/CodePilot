...
/**
 * WebFilter that authenticates admin API requests via a static API key.
 * The key is provided in the {@code X-Admin-API-Key} header and compared
 * against the configured {@code codepilot.admin.api-key} property.
...
public class AdminAuthFilter implements WebFilter {
    ...
    public AdminAuthFilter(
            @Value("${codepilot.admin.api-key:}") String adminApiKey) {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        if (!MessageDigest.isEqual(
            ...
            log.warn("Invalid admin API key attempt from {}",
    ...
    private Mono<Void> sendError(ServerWebExchange exchange, int code, String message) {
        HttpStatus status = code == ErrorCodes.UNAUTHORIZED ? HttpStatus.UNAUTHORIZED : HttpStatus.FORBIDDEN;
