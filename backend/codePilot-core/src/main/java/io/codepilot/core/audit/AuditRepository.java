...
/**
 * Audit repository for persisting audit events to PostgreSQL. All audit events are stored in
 * audit_events table with traceId for cross-service correlation. Never stores raw user code content
 * - only paths, tool names, and metadata. All writes are async to avoid blocking the request path.
 */
@Repository
public class AuditRepository {
  ...
  public void saveEvent(
    ...
    try {
      ...
      jdbc.update(
          "INSERT INTO audit_events (id, trace_id, session_id, action, actor, payload,"
              + " privacy_mode, created_at) VALUES (?, ?, ?, ?, ?, CAST(? AS JSON), ?, CAST(? AS"
              + " DATETIME))",
          id,
          traceId,
          sessionId,
  ...
  @Async
  public void saveSkillActivation(
      String traceId,
      String sessionId,
      String skillId,
      String skillVersion,
  ...
  public void saveSecurityEvent(
    ...
    String payload =
        String.format("{\"eventType\":\"%s\",\"detail\":\"%s\"}", eventType, safeDetail);
