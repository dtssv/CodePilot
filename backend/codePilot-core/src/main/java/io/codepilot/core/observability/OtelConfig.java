 ...
 *
 * <p>Enabled when {@code codepilot.otel.enabled=true} in application config. Exports traces to an
 * OTLP-compatible backend (Jaeger, Tempo, etc.) and metrics to Prometheus/Grafana via OTLP.
 *
 * <p>traceId is propagated from the plugin through the gateway to all services, enabling full
...
public class OtelConfig {
  ...
  public SdkMeterProvider sdkMeterProvider(OtlpGrpcMetricExporter metricExporter) {
    Resource resource = Resource.create(Attributes.of(ServiceAttributes.SERVICE_NAME, serviceName));
    return SdkMeterProvider.builder()
        .setResource(resource)
        .registerMetricReader(
            PeriodicMetricReader.builder(metricExporter).setInterval(exportInterval).build())
        .build();
  }
  ...
  public OpenTelemetry openTelemetry(
      SdkTracerProvider tracerProvider, SdkMeterProvider meterProvider) {
