 ...
 *
 * <p>MCP server addresses and tool metadata are derived from the {@code userMcps[]} field
 * in the conversation request. The backend does NOT manage MCP server processes — that is
 * the plugin's responsibility. For SSE/Streamable HTTP MCP servers, the backend connects
 * directly; for stdio-based servers, the call must be routed back to the plugin (client-side).
 *
 * <h3>Transport Modes:</h3>
 * <ul>
 *   <li><b>Streamable HTTP</b>: Direct HTTP POST with JSON-RPC — backend can call directly</li>
 *   <li><b>SSE</b>: HTTP POST to the server's message endpoint — backend can call directly</li>
 *   <li><b>stdio</b>: Requires local process — backend delegates back to client-side</li>
 * </ul>
 */
@Component
public class McpToolExecutor {
    ...
    private final ConcurrentHashMap<String, List<Map<String, Object>>> sessionToolCache = new ConcurrentHashMap<>();
    ...
    /**
     * Register MCP tool metadata for a session. Called after the plugin sends the
     * tools/list results from each MCP server.
     */
     ...
     * @param serverId  the MCP server ID (extracted from the tool name, e.g. "weather" from "mcp.weather.query")
    ...
    public String execute(String sessionId, String serverId, String toolName, JsonNode args) {
        ...
        UserMcp mcp = mcps.stream()
                .filter(m -> serverId.equals(m.id()))
                .findFirst()
                .orElse(null);
        ...
        if ("stdio".equals(transport)) {
            return errorJson("stdio MCP server '" + serverId + "' requires client-side execution. "
        ...
        try {
            ...
            if (response.statusCode() >= 200 && response.statusCode() < 300) {
                ...
                return mapper.writeValueAsString(Map.of(
                        ...
                        "serverId", serverId,
                        "toolName", toolName,
                        "result", result != null ? mapper.treeToValue(result, Object.class) : null
                ));
            } else {
                return errorJson("MCP server HTTP error: " + response.statusCode()
    ...
    /**
     * Build a list of available MCP tool descriptors for prompt injection.
     * Format: each entry has "fullName" (e.g. "mcp.weather.query"), "description", and "parameters".
     */
