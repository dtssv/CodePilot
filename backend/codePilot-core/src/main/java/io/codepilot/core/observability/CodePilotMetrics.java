 ...
 *
 * Counters:
 *   - codepilot_tokens_in:    Total input tokens consumed
 *   - codepilot_tokens_out:   Total output tokens generated
 *   - codepilot_agent_steps_total:  Total agent steps executed
 *   - codepilot_tool_calls_total:   Total tool calls invoked
 *
 * Histogram/Timer:
 *   - codepilot_latency_seconds:    End-to-end latency distribution
 */
@Component
public class CodePilotMetrics {
    ...
    public CodePilotMetrics(MeterRegistry registry) {
        this.tokensIn = Counter.builder("codepilot_tokens_in")
                .description("Total input tokens consumed")
                .register(registry);

        this.tokensOut = Counter.builder("codepilot_tokens_out")
                .description("Total output tokens generated")
                .register(registry);

        this.agentStepsTotal = Counter.builder("codepilot_agent_steps_total")
                .description("Total agent steps executed")
                .register(registry);

        this.toolCallsTotal = Counter.builder("codepilot_tool_calls_total")
                .description("Total tool calls invoked")
                .register(registry);

        this.latencyTimer = Timer.builder("codepilot_latency_seconds")
    ...
    public void recordLatency(double seconds) {
        latencyTimer.record((long) (seconds * 1_000_000_000), java.util.concurrent.TimeUnit.NANOSECONDS);
    }
