 ...
 *       available to all users. These have their own base_url, model name, and api_key.
 *   <li><b>Custom models</b> — per-user models in {@code custom_model_providers}, only the owning
 *       user can view/update/delete.
 * </ul>
 *
...
public class ModelService {
  ...
  public CustomModelProvider update(UUID id, String userId, UpdateModelCommand req) {
    ...
    if (req.name() != null) {
      ...
      params.addValue("name", req.name());
    }
    if (req.protocol() != null) {
      set.append("protocol = :protocol, ");
      params.addValue("protocol", req.protocol());
    }
    if (req.baseUrl() != null) {
      set.append("base_url = :baseUrl, ");
      params.addValue("baseUrl", req.baseUrl());
    }
    if (req.apiKey() != null) {
      set.append("api_key_cipher = :cipher, ");
      params.addValue("cipher", encrypt(req.apiKey()));
    }
    if (req.model() != null) {
      set.append("model = :model, ");
      params.addValue("model", req.model());
    }
    if (req.headers() != null) {
      set.append("headers_json = :headers, ");
      params.addValue("headers", toJson(req.headers()));
    }
    if (req.timeoutMs() != null) {
      set.append("timeout_ms = :timeout, ");
      params.addValue("timeout", req.timeoutMs());
    }
    if (req.enabled() != null) {
      set.append("enabled = :enabled, ");
      params.addValue("enabled", req.enabled());
    }
  ...
  public void delete(UUID id, String userId) {
    ...
    int rows =
        jdbc.update(
            sql,
            new MapSqlParameterSource().addValue("id", id.toString()).addValue("userId", userId));
    if (rows == 0) throw new CodePilotException(ErrorCodes.NOT_FOUND, "Model not found: " + id);
  }

  /**
   * List all models available to a user: model groups (replacing system models) + user's custom
   * models. Model groups are the new user-facing concept; each group contains multiple app keys for
   * load balancing, but users only see the group-level info.
   */
  ...
  private List<String> normalizeCapabilities(String model, List<String> raw) {
    ...
    if (id.contains("vision")
        ...
        || id.contains("gpt-5")
        || id.contains("claude")
        || id.contains("gemini")) caps.add("VISION");
    if (id.contains("gpt") || id.contains("claude") || id.contains("gemini")) caps.add("JSON_MODE");
    if (id.contains("200k") || id.contains("256k") || id.contains("claude"))
      caps.add("LONG_CTX_256K");
  ...
  private String inferTier(String model, List<String> capabilities, int maxTokens) {
    String id = model == null ? "" : model.toLowerCase(java.util.Locale.ROOT);
    if (id.contains("haiku") || id.contains("flash") || id.contains("mini") || id.contains("fast"))
      return "FAST";
    if (id.contains("opus") || id.contains("max") || id.contains("gpt-5")) return "PREMIUM";
    if (id.contains("thinking") || id.contains("reason") || id.contains("sonnet"))
      return "THINKING";
  ...
  @Transactional
  public ModelGroup updateModelGroup(
      UUID id,
      String name,
      String protocol,
      String baseUrl,
    ...
    if (baseUrl != null) {
      set.append("base_url = :baseUrl, ");
      params.addValue("baseUrl", baseUrl);
    }
    if (model != null) {
      set.append("model = :model, ");
    ...
    if (capabilities != null) {
      set.append("capabilities = :caps, ");
      params.addValue("caps", toJson(capabilities));
    }
    if (maxTokens != null) {
      set.append("max_tokens = :maxTokens, ");
      params.addValue("maxTokens", maxTokens);
    }
    if (timeoutMs != null) {
      set.append("timeout_ms = :timeoutMs, ");
      params.addValue("timeoutMs", timeoutMs);
    }
    if (enabled != null) {
      set.append("enabled = :enabled, ");
    ...
    if (sortOrder != null) {
      set.append("sort_order = :sortOrder, ");
    ...
    int rows = jdbc.update(sql, params);
    if (rows == 0)
      throw new CodePilotException(ErrorCodes.NOT_FOUND, "Model group not found: " + id);
    return findModelGroupById(id);
  }
  ...
  @Transactional
  public ModelAppKey createAppKey(
      UUID groupId,
      String name,
      String baseUrl,
      String apiKey,
  ...
  /** Update an app key. */
  @Transactional
  public ModelAppKey updateAppKey(
      UUID id,
      String name,
      String baseUrl,
      String apiKey,
    ...
    if (apiKey != null) {
      set.append("api_key_cipher = :cipher, ");
      params.addValue("cipher", encrypt(apiKey));
    }
    if (weight != null) {
      set.append("weight = :weight, ");
      params.addValue("weight", weight);
    }
    if (maxConcurrency != null) {
      set.append("max_concurrency = :maxConcurrency, ");
      params.addValue("maxConcurrency", maxConcurrency);
    }
    if (rpmLimit != null) {
      set.append("rpm_limit = :rpmLimit, ");
      params.addValue("rpmLimit", rpmLimit);
    }
    if (tpmLimit != null) {
      set.append("tpm_limit = :tpmLimit, ");
      params.addValue("tpmLimit", tpmLimit);
    }
    if (priority != null) {
      set.append("priority = :priority, ");
  ...
  private void verifyOwnership(UUID modelId, String userId) {
    ...
    if (!ownerId.equals(userId)) {
      throw new CodePilotException(
          ErrorCodes.FORBIDDEN, "Cannot modify model owned by another user");
    }
  }
  ...
  byte[] encrypt(String plaintext) {
    try {
      ...
      cipher.init(
          Cipher.ENCRYPT_MODE, new SecretKeySpec(kmsKey, "AES"), new GCMParameterSpec(128, iv));
