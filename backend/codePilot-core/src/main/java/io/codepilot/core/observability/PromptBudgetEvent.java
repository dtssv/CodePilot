 ...
 *
 * <p>Tracks: - How many tokens were budgeted vs consumed per request - Which context layers were
 * pruned (and how many tokens saved) - Whether LazyRef resolution was budget-limited - Compression
 * ratio when digest is applied
 *
 * <p>Events are stored in a ring buffer (last 1000) and exposed via MetricsController for
 * Prometheus scraping and debugging.
 */
public class PromptBudgetEvent {
  ...
  public static void record(
    ...
    log.debug(
        "PromptBudget: session={} mode={} budget={} consumed={} pruned={} lazyRef={}/{}"
            + " compression={}",
        sessionId,
        mode,
        budgetTokens,
        ...
        prunedTokens,
        lazyRefResolved,
        lazyRefResolved + lazyRefBudgetLimited,
  ...
  public static Record[] recentEvents(int limit) {
    return ringBuffer.stream().skip(Math.max(0, ringBuffer.size() - limit)).toArray(Record[]::new);
  }
  ...
  public static Map<String, Number> aggregateStats() {
    ...
    double avgCompression =
        ringBuffer.stream().mapToDouble(Record::compressionRatio).average().orElse(0);
    long budgetLimited =
        ringBuffer.stream().mapToInt(Record::lazyRefBudgetLimited).filter(i -> i > 0).count();
